...
class TurtleBot:
    ...
    def distanciaMinima(self,x,y): #Esta función no sirve para detectar tortugas en el semicirculo superior; sirve para hacer el test de la distancia mínima al final de cada trayectoria
        return min([self.calculaDistancia(x,y,x1,y1), self.calculaDistancia(x,y,x3,y3), self.calculaDistancia(x,y,x4,y4), self.calculaDistancia(x,y,x5,y5), self.calculaDistancia(x,y,x6,y6)])
    ...
    def seleccionaTrayectoria(self,trayectorias): #De la lista de trayectorias generadas anteriormente, ¿cuál es la mejor? Si regresa un -1 lo mejor es esperar y recalcular
        ...
        for i in trayectorias:
            aux = min([self.calculaDistancia(i[0],i[1],x1,y1),self.calculaDistancia(i[0],i[1],x3,y3),self.calculaDistancia(i[0],i[1],x4,y4),self.calculaDistancia(i[0],i[1],x5,y5),self.calculaDistancia(i[0],i[1],x6,y6)]);
            if aux > mini and i[0] > 1.5 and i[0] < 9.5 and i[1] > 1.5 and i[1] < 9.5 and abs(i[0]-self.pose.x)>0.5 and abs(i[1]-self.pose.y)>0.5:
                ...
                aux2 = min([self.calculaDistancia(xmid,ymid,x1,y1),self.calculaDistancia(xmid,ymid,x3,y3),self.calculaDistancia(xmid,ymid,x4,y4),self.calculaDistancia(xmid,ymid,x5,y5),self.calculaDistancia(xmid,ymid,x6,y6)]);
                if aux2 > 1:
                    mini = aux;
                    res = i;
        return res
...
if __name__ == '__main__':
    ...
    recorrePuntos([[1.5,10],[10,4],[5.5,9]],0.1)
