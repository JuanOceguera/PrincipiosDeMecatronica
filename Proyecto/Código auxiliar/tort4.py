...
class TurtleBot:
    ...
    def __init__(self):
        ...
        # Publisher which will publish to the topic '/turtle4/cmd_vel'.
        self.velocity_publisher = rospy.Publisher('/turtle4/cmd_vel',
                                                  Twist, queue_size=10)

        # A subscriber to the topic '/turtle4/pose'. self.update_pose is called
        # when a message of type Pose is received.
        self.pose_subscriber = rospy.Subscriber('/turtle4/pose',
    ...
    def distanciaMinima(self,x,y): #Esta función no sirve para detectar tortugas en el semicirculo superior; sirve para hacer el test de la distancia mínima al final de cada trayectoria
        return min([self.calculaDistancia(x,y,x1,y1), self.calculaDistancia(x,y,x2,y2), self.calculaDistancia(x,y,x3,y3), self.calculaDistancia(x,y,x5,y5), self.calculaDistancia(x,y,x6,y6)])
    ...
    def seleccionaTrayectoria(self,trayectorias): #De la lista de trayectorias generadas anteriormente, ¿cuál es la mejor? Si regresa un -1 lo mejor es esperar y recalcular
        ...
        for i in trayectorias:
            aux = min([self.calculaDistancia(i[0],i[1],x1,y1),self.calculaDistancia(i[0],i[1],x2,y2),self.calculaDistancia(i[0],i[1],x3,y3),self.calculaDistancia(i[0],i[1],x5,y5),self.calculaDistancia(i[0],i[1],x6,y6)]);
            if aux > mini and i[0] > 1.5 and i[0] < 9.5 and i[1] > 1.5 and i[1] < 9.5 and abs(i[0]-self.pose.x)>0.5 and abs(i[1]-self.pose.y)>0.5:
                ...
                aux2 = min([self.calculaDistancia(xmid,ymid,x1,y1),self.calculaDistancia(xmid,ymid,x2,y2),self.calculaDistancia(xmid,ymid,x3,y3),self.calculaDistancia(xmid,ymid,x5,y5),self.calculaDistancia(xmid,ymid,x6,y6)]);
                if aux2 > 1:
                    mini = aux;
                    res = i;
        return res
...
if __name__ == '__main__':
    ...
    recorrePuntos([[4.5,10],[10,2],[9.5,5]],0.1)
